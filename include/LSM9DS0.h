...
typedef struct {
    int16 gx;
	int16 gy;
 	int16 gz; // x, y, and z axis readings of the gyroscope
	int16 ax;
    int16 ay;
    int16 az; // x, y, and z axis readings of the accelerometer
	int16 mx;
	int16 my;
    int16 mz; // x, y, and z axis readings of the magnetometer
    int16 temperature;
	float abias[3];
    float gbias[3];
} LSM_properties;
...
typedef struct {
    enum gyro_scale gScl;
    enum accel_scale aScl;
    enum mag_scale mScl;
	enum gyro_odr gODR;
	enum accel_odr aODR;
	enum mag_odr mODR;
